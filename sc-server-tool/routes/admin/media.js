...
// API đổi tên ảnh
router.patch("/images/:imageId/rename", async function (req, res) {
  try {
    const Image = getModel("Image");
    const imageId = req.params.imageId;
    let newImageName = req.body.newName;

    // Kiểm tra tên mới đã tồn tại trong cơ sở dữ liệu chưa (bao gồm cả phần mở rộng)
    const existingImage = await Image.findOne({ name: newImageName });
    if (existingImage && existingImage._id.toString() !== imageId) {
      return res
        .status(409)
        .send({ error: "Image with this name already exists" });
    }
    ...
    const imageDir = path.dirname(image.path);

    // Tạo đường dẫn mới cho ảnh (bao gồm cả phần mở rộng)
    const newImagePath = path.join(imageDir, newImageName);

    // Đổi tên file ảnh trong hệ thống file
    await fs.promises.rename(image.path, newImagePath);

    // Cập nhật đường dẫn và tên ảnh trong cơ sở dữ liệu
    image.path = newImagePath;
    image.name = newImageName;
    await image.save();

    return res.send({ message: "Image renamed successfully", image });
  } catch (e) {
    console.log("error", e);
    res.status(500).send({ error: e });
  }
