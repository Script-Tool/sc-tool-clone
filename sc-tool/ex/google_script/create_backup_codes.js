async function createBackupCodes(action) {
  ...
  try {
    switch (true) {
      ...
      case url.includes(
        ...
        await sleep(2000);
        await handleCreateBackupCodes(action);
        break;
      }
...
async function handleCreateBackupCodes() {
  const createCodeBtn = getElementContainsInnerText(
    "span",
    ["Get backup codes"],
    "",
    "equal"
  ...
  if (createCodeBtn) {
    await userClick(action.pid, 'button[jsname="Pr7Yme"]', createCodeBtn);

    await sleep(2000);
  } else {
    await userClick(action.pid, '[aria-label="Generate new codes"]');
    await sleep(2000);
  ...
  const backupCodes = [
    ...document.querySelectorAll('.h7muCf div[dir="ltr"]'),
  ].map((ele) => ele.textContent);

  action.data_reported = backupCodes.join("|");
  await updateProfileData({
    pid: action.pid,
    backup_code: backupCodes.join("|"),
    is_reset_backup_code: false,
  });
