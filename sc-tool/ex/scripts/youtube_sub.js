async function scriptYoutubeSub(action) {
    try {
        ...
        } else if (url.indexOf('https://seo.s9s.ai') > -1) {
            await userClick(action.pid, '', getElementContainsInnerText('a', ['Visit Website']));
        }
    } catch (e) {
...
async function processHomePageSub(action) {
    ...
    await goToLocation(action.pid, 'https://seo.s9s.ai/#website/sn-nguyn-99-youtube');
    return;
    ...
    if (action.channel_id) {
        ...
        if (!action.channel_id.startsWith('channel/') && !action.channel_id.startsWith('@')) {
            channelLink = 'channel/' + action.channel_id;
        }
        await goToLocation(action.pid, 'https://www.youtube.com/' + channelLink + '/videos');
        return;
    }
...
async function skipAdsSub(watchingCheck, action = {}) {
    ...
    while (document.querySelector('.ytp-ad-skip-ad-slot')) {
        ...
        while (
            ...
            !document.querySelector('button.ytp-ad-skip-button').getBoundingClientRect().x
        ) {
            await sleep(1000);
        }
